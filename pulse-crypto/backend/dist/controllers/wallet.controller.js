...
exports.listReferredUsers = exports.getUserStats = exports.applyReferralCode = exports.updateTokenBalances = exports.getOrCreateUser = void 0;
 ...
 */
const updateTokenBalances = async (req, res) => {
    try {
        const { walletAddress } = req.params;
        ...
        // Find the user
        const user = await user_model_1.default.findOne({ walletAddress });
        if (!user) {
            res.status(404).json({
                success: false,
        ...
        if (tokens) {
            if (tokens.BeTyche !== undefined)
                user.tokens.BeTyche = tokens.BeTyche;
            if (tokens.ETH !== undefined)
                user.tokens.ETH = tokens.ETH;
            if (tokens.RADBRO !== undefined)
                user.tokens.RADBRO = tokens.RADBRO;
...
const getUserStats = async (req, res) => {
    try {
        ...
        const referralCount = await user_model_1.default.countDocuments({ referredBy: walletAddress });
 ...
 */
const listReferredUsers = async (req, res) => {
    try {
        const { walletAddress } = req.params;
        ...
        // Find referred users
        const referredUsers = await user_model_1.default
        ...
        const total = await user_model_1.default.countDocuments({ referredBy: walletAddress });
        res.status(200).json({
            success: true,
            data: {
                referredUsers,
...
exports.listReferredUsers = listReferredUsers;
